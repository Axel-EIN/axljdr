    ...
    public function addFamille(Request $request, EntityManagerInterface $em, ClanRepository $clanRepository, FileHandler $fileHandler) {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (!empty($nouveauMon)) {
                ...
                $famille->setMon($fileHandler->handle($nouveauMon, null, $prefix, 'familles', 'square320'));
            }
    ...
    public function editFamille(Request $request, Famille $famille, FileHandler $fileHandler): Response {
        ...
        if($form->isSubmitted() && $form->isValid()) {
            ...
            if (!empty($nouveauMon)) {
                ...
                $famille->setMon($fileHandler->handle($nouveauMon, $famille->getMon(), $prefix, 'familles', 'square320'));
            } elseif ($request->request->get('remove_mon') === '1' && $famille->getMon()) {
