...
class Visibility
{
    ...
    public function state($element, bool $revealAuto = false): string
    {
        ...
        if (Access::needsUnlock($access) && ($this->isUnlocked($element) || $this->isOwned($element))) {
    ...
    private function owned(): array
    {
        ...
        foreach ($this->currentPlayer->characters() as $personnage) {
            foreach ([$personnage->getEcole()] as $element) {
                $key = EntityRegistry::keyOf($element);
