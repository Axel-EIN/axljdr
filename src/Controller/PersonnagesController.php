...
class PersonnagesController extends AbstractController
{
    ...
    public function viewPersonnages(
        ...
        if ($request->query->has('saison')) {
            ...
            $latestSaison = $saisonRepository->findOneBy([], ['numero' => 'DESC']);
    ...
    public function viewPersonnageProfil(Personnage $personnage, PersonnageRepository $personnageRepository): Response {
        ...
        // Personnage verrouillé : réservé au MJ. Les joueurs voient une page "à découvrir".
        if ($personnage->getLocked() && !$this->isGranted('ROLE_MJ')) {
            return $this->render('personnages/profil.html.twig', [
                'personnage' => $personnage,
                'nom' => 'Personnage à découvrir',
                'entity' => 'personnage',
    ...
    public function afficherFichePersonnage(FichePersonnage $fiche, CompetenceRepository $competenceRepository, AvantageRepository $avantageRepository, ObjetRepository $objetRepository): Response
    {
        ...
        return $this->render('personnages/fiche-personnage.html.twig', [
            'fiche'          => $fiche,
            'xp_total'       => $xp_total,
            'xp_creation'    => $xp_creation,
            'xp_progression' => $xp_progression,
