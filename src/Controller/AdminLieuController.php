...
class AdminLieuController extends AbstractController
{
  ...
  public function addLieu(Request $request, EntityManagerInterface $em, FileHandler $fileHandler, Baliseur $baliseur) {
    ...
    if ($form->isSubmitted() && $form->isValid()) {
      ...
      if (!empty($nouvelleImage)) {
        ...
        $lieu->setImage($fileHandler->handle($nouvelleImage, null, $prefix, 'lieux', 'landscape720'));
      }
  ...
  public function editLieu(Request $request, Lieu $lieu, FileHandler $fileHandler, Baliseur $baliseur): Response {
    ...
    if($form->isSubmitted() && $form->isValid()) {
      ...
      if (!empty($nouvelleImage)) {
        $prefix = 'lieu-' . $lieu->getNom() . '-image';
        $lieu->setImage($fileHandler->handle($nouvelleImage, $lieu->getImage(), $prefix, 'lieux', 'landscape720'));
      } elseif ($request->request->get('remove_image') === '1' && $lieu->getImage()) {
      ...
      if (!empty($nouvelleIcone)) {
        $prefix = 'lieu-' . $lieu->getNom() . '-icone';
        $lieu->setIcone($fileHandler->handle($nouvelleIcone, $lieu->getIcone(), $prefix, 'lieux', 'square192'));
      } elseif ($request->request->get('remove_icone') === '1' && $lieu->getIcone()) {
